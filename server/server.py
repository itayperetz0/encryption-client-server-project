...
class Server:
    ...
    # Collect port number from port.info file
    ...
    #gets request payload nad header   
    ...
    def handle_client_requests(self, client_socket, client_address,clients): 
            ...
            while True:
                ...
                if code == 1027:
                    ...
                    if client is None:
                        ...
                        self.handle_1602_and_1605(client_socket, client,1605) # to check why failed and stuck on this line in client 
                ...
                if code == 1028:   
                    ...
                    if current_packet_num <= packets_number:
                        if current_packet_num == 1:##case first packet
                            ...
                            if current_packet_num > len(client['files'][file_name]['Content'])+1:
                                ...
                                client_socket.sendall(self.net_protocol.res_1607(client_id))
                                continue
                            else:#case packet number as expected
                                if len(client['files'][file_name]['Content']) == current_packet_num :#case it is not first time of this packet
                                    client['files'][file_name]['Content'][current_packet_num-1] = encrypted_massege_content
                                else:#case first time sending packet
                                    client['files'][file_name]['Content'].append(encrypted_massege_content)
                                    
                    if current_packet_num == packets_number:#case all packets sent
                        try:
        ...
        finally: #when client finished his requests 
            print(f"User finished his requests closing connection")                           
            client_socket.close()
            return self.clients   
    
    #decrypt client file content 
