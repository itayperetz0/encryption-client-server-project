...
/**
 * @brief Protocol constants for network communication
 */
#define CLIENT_VER 3
#define CRC_LEN 4
#define UUID_LEN 16
...
int Network::recive_1600_and_1601_res(char* client_uuid) {
    ...
    std::cout << "1600 - registered successfully";
    return 1600;
}
...
int Network::recive_1603_and_1607_res(unsigned int& server_crc) {
    ...
    if (code == 1603) {
        ...
        std::string uuid(buffer, 16);
        unsigned int content_size;
        std::memcpy(&content_size, buffer + 16, sizeof(content_size));
        char file_name[255];
        std::memcpy(file_name, buffer + 20, 255);
        unsigned int crc;
        std::memcpy(&crc, buffer + 275, sizeof(crc));
        server_crc = crc;
        return 1603;
    }
    if (code == 1607) {
...
bool Network::recive_1604_res(std::string client_uuid) {
    ...
    if (code == 1604) {
        ...
        if (uuid == client_uuid.substr(0, UUID_LEN)) {
            std::cout << "got 1604 - server received 1029 or 1031 request successfully" << std::endl;
            return true;
        }
        return false;
    }
...
int Network::recive_1605_and_1606_res(char* client_uuid, std::string& private_key, std::string& files_key) {
    ...
    uint32_t payload_size = *(uint32_t*)(buffer + 3);
    if (code == 1606) {
        std::cout << "1606 - failed to reconnect to server - client do not registered in server" << std::endl;
        char payload[20] = {};
        bytes_rec = recv(this->client_socket, payload, payload_size, 0);
        strncpy(client_uuid, payload, UUID_LEN);
        return 1606;
...
bool Network::req_1026(std::string client_name, std::string client_uuid, std::string public_key, std::string& private_key, std::string& files_key) {
    ...
    std::cout << "AES key received encrypted from server and after decryption is: " << Base64Wrapper::encode(files_key) << std::endl;
    return true;
}
...
int Network::req_1027(std::string client_name, char* client_uuid, std::string& private_key, std::string& files_key) {
    ...
    if (!rec_1605_and_1606_res) {
        return 0;
    }
    if (rec_1605_and_1606_res == 1606) {
        std::cout << "reconnect to server failed - client registered as new client" << std::endl;
        return 1606;
    }
